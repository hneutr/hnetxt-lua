...
local function setup_entry_sets()
    ...
    prompt = PromptEntry(key, {response_key = response_key}, {}, test_project_root)
...
describe("respond", function()
    it("works", function()
        setup_entry_sets()
        assert.are.same(test_response_1_2, prompt:respond(test_prompt_1))
        assert.are.same(
...
describe("response", function()
    it("all", function()
        setup_entry_sets()
        response:new_entry(test_response_1_1, {date = Path.stem(test_response_1_1)})
        response:new_entry(test_response_1_2, {pinned = true, date = Path.stem(test_response_1_2)})
